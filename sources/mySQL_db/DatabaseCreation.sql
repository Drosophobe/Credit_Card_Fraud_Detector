...
DROP TABLE IF EXISTS ccf_data.ccf_data_full;
CREATE TABLE ccf_data.ccf_data_full (
    ...
    fraud INTEGER);
LOAD DATA INFILE '/Users/pierresarzier/Credit_Card_Fraud/sources/dataset_csv/card_transdata.csv'
...
CREATE TABLE ccf_data.ccf_data_partial (
    distance_from_home FLOAT,
    distance_from_last_transaction FLOAT,
    ratio_to_median_purchase_price  FLOAT,
    repeat_retailer INTEGER,
    ...
    online_order INTEGER,
    fraud INTEGER);
INSERT INTO ccf_data.ccf_data_partial SELECT * FROM ccf_data.ccf_data_full LIMIT 1000;
-- We are checking the Table previously created
SELECT * FROM ccf_data.ccf_data_partial;
SELECT COUNT(*) FROM ccf_data.ccf_data_partial;


-- We are copying the table with 0.1% of our data which will be used for train the new model and updated during production
DROP TABLE IF EXISTS ccf_data.ccf_data_i;
CREATE TABLE ccf_data.ccf_data_i (
    distance_from_home FLOAT,
    distance_from_last_transaction FLOAT,
    ratio_to_median_purchase_price  FLOAT,
    repeat_retailer INTEGER,
    ...
    online_order INTEGER,
    fraud INTEGER);
INSERT INTO ccf_data.ccf_data_i SELECT * FROM ccf_data.ccf_data_partial;
-- We are checking the Table previously created
SELECT * FROM ccf_data.ccf_data_i LIMIT 0, 1000000;
SELECT COUNT(*) FROM ccf_data.ccf_data_i;


-- We are creating a empty table that will be upaded with the corrected predictions to train model into production 
DROP TABLE IF EXISTS ccf_data.ccf_data_to_add;
CREATE TABLE ccf_data.ccf_data_to_add (
    distance_from_home FLOAT,
    distance_from_last_transaction FLOAT,
    ratio_to_median_purchase_price  FLOAT,
    repeat_retailer INTEGER,
    ...
    fraud INTEGER);
-- We are checking the Table previously created
SELECT * FROM ccf_data.ccf_data_to_add;
-- Just a way to delte Table while testing
DELETE FROM ccf_data.ccf_data_to_add;
...
DROP TABLE IF EXISTS ccf_data.ccf_data_remaining;
CREATE TABLE ccf_data.ccf_data_remaining (
    distance_from_home FLOAT,
    distance_from_last_transaction FLOAT,
    ratio_to_median_purchase_price  FLOAT,
    repeat_retailer INTEGER,
    ...
    online_order INTEGER,
    fraud INTEGER);
INSERT INTO ccf_data.ccf_data_remaining SELECT * FROM ccf_data.ccf_data_full;
