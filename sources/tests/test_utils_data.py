    ...
    def test_db_write(self):
        with self.mock_db_config:
            for table in ["ccf_data_full", "ccf_data_partial", "ccf_data_i", "ccf_data_to_add", "ccf_data_remaining"]:
                print("111")
                self.assertEqual(db_utils_data.db_write(f"""INSERT INTO `{table}` (`distance_from_home`, `distance_from_last_transaction`, `ratio_to_median_purchase_price`, `repeat_retailer`, `user_chip`, `used_pin_number`, `online_order`, `fraud`) VALUES
